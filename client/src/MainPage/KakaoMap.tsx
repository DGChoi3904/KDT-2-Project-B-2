...
function KakaoMap() {
  ...
  const [duration, setDuration] = useState<number>(0); //소요 시간
  ...
  const handleNavi = () => {
    ...
    fetch(url, {
      ...
      .then((jsonData) => {
        //소요시간
        const duration = jsonData.routes[0].sections[0].duration;
        // 요청에 대한 처리
        ...
        setDataCheck(true);
        // 소요 시간 업데이트
        const durationInSeconds = jsonData['routes'][0]['duration'];
  ...
  return (
    <div>
      ...
      {duration > 0 && <div>소요 시간: {duration}초</div>}{' '}
      {/* 소요 시간 출력 */}
